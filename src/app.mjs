import {
  PROJECT_STATUSES, REQUIRED_PROJECT_ROLES, DEPARTMENTS, POSITIONS, SKILL_OPTIONS, SKILL_LEVELS, EMPLOYMENT_STATUSES, CAPABILITY_UNIT_SUGGESTIONS, DEFAULT_DICTIONARIES,
...
function renderImport() {
  $('#view-import').innerHTML = `
    ...
    <div class="import-grid">
      <div class="card import-card"><div class="import-icon">▦</div><h3>项目资料</h3><p>导入项目基本信息、进度、客户、资料路径和各环节人员。重名项目将更新已有资料。</p><div class="import-actions">${manageButton('<button class="btn btn-primary" data-action="import-projects">选择文件导入</button>')}<button class="btn btn-outline" data-template="projects">下载模板</button></div></div>
...
const dictionaryLabels={departments:'部门',positions:'职位',skills:'技能标签',projectStatuses:'项目状态',employmentStatuses:'在岗状态'};
const dictionaryDefaults={departments:DEPARTMENTS,positions:POSITIONS,skills:SKILL_OPTIONS,projectStatuses:PROJECT_STATUSES,employmentStatuses:EMPLOYMENT_STATUSES};
...
function projectForm(project = {}) {
  ...
  return projectFields.map(original => {const field=original[0]==='status'?[...original.slice(0,4),dictionary('projectStatuses',PROJECT_STATUSES)]:original;return `${sectionBefore[field[0]]?`<div class="form-section">${sectionBefore[field[0]]}</div>`:''}${inputField(field, project[field[0]])}`;}).join('')+customFieldInputs('projects',project.customValues);
}
...
function editAssignment({projectId='',personId='',needId='',role=''}={}) {
  ...
  openModal({title:requestedRole?`安排${requestedRole}`:'添加项目人员',subtitle:'展示全部人员；满载、超载或非在岗人员仍可选择，提交前会提示风险',size:'medium',body:`<form id="assignment-form" class="form-grid">
    ...
    <div class="field full"><label>人员<em>*</em></label><select name="personId">${candidateList.map(person=>`<option value="${person.id}" ${personId===person.id?'selected':''}>${esc(person.name)} · ${esc(positionsText(person))} · ${person.employmentStatus!=='在岗'?esc(person.employmentStatus):personAvailable(db,person)>0?`可用 ${personAvailable(db,person)}%`:personRemainingCapacity(db,person)<0?`超载 ${Math.abs(personRemainingCapacity(db,person))}%`:'已满载'}</option>`).join('')}</select></div>
